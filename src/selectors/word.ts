...
const WordSelectors = {
	wordData: createSelector([getWord], (word) => {
		...
		const prefixOfWord = WordComputers.unprefixed1Part(wordFormatted);
		const suffixOfWord = WordComputers.unsuffixed1Part(wordFormatted);
		...
		return {
			...
			prefixes1: ods8Words
				...
				.map((ods8Word) => ({ word: ods8Word, prefix: WordComputers.prefix1(ods8Word) })),

			prefixOf: { word: prefixOfWord, valid: ods8Words.includes(prefixOfWord) },

			suffixes1: ods8Words
				.filter((ods8Word) => suffixes1Regex.test(ods8Word))
				.map((ods8Word) => ({ word: ods8Word, suffix: WordComputers.suffix1(ods8Word) })),
